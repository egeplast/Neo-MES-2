...
export class MsalGraphService {
    ...
    this.getAccessToken(["https://graph.microsoft.com/User.Read"])
      .pipe(
        switchMap((token) => {
          ...
          return this.http.get<GraphUserInfo>(
  ...
  loadProfilePicture(account?: AccountInfo): void {
    ...
    this.getAccessToken(["https://graph.microsoft.com/User.Read"])
      ...
      .subscribe((base64Picture) =>
        this.profilePictureSubject.next(base64Picture),
      );
  }

  getProfilePicture(): Observable<string> {
    if (!this.profilePictureSubject.value && this.userLoaded)
